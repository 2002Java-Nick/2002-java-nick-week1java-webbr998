...
public class EvaluationService {
	...
	public String cleanPhoneNumber(String string) {
		...
		String noWhiteSpace = noPlus.replaceAll(" ", "");

		String clipped = noWhiteSpace;
		if(noWhiteSpace.charAt(0) == '1') {
			clipped = noWhiteSpace.substring(1);
		}
		return clipped;
	}
	...
	public boolean isArmstrongNumber(int input) {
		int inputMagnitude = input;
		int places = 0;
		while(inputMagnitude > 0) {
			inputMagnitude /= 10;
			places++;
		}
		...
		while(input > 0) {
			tmp = input % 10;
			runningSum += (int) Math.round(Math.pow(tmp,places));
			input /= 10;
		}
		...
		public String rotate(String string) {
			...
			for(int i=0; i<len; i++) {
				...
				if(Character.isLetter(c)) {
					c = (char) (string.charAt(i) + RotationalCipher.getKey());
					if(c > 'z') {
						result += (char) (string.charAt(i) - (26-RotationalCipher.getKey()))
					}
					else {
						result += (char) (string.charAt(i) + RotationalCipher.getKey());
					}
				}
			}
			return result;
